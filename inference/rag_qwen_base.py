...
class QwenBaseRAGInference:
    """베이스 Qwen 모델을 사용한 RAG 추론 클래스"""
    ...
    def create_rag_prompt(self, query: str, selected_doc_ids: List[int]) -> str:
        ...
        # 베이스 모델용 프롬프트
        system_prompt = """당신은 한국어 맞춤법과 문장부호 교정 전문가입니다. 주어진 참고 문서들을 활용하여 질문에 정확하게 답변하세요.

참고 문서를 바탕으로:
- 교정형 문제: 틀린 부분을 찾아 올바르게 고치고, 근거를 제시하세요.
- 선택형 문제: 올바른 답을 선택하고, 이유를 설명하세요.

답변은 명확하고 자연스럽게 작성하세요."""
    ...
    def generate_answer(self, prompt: str, max_new_tokens: int = 256) -> str:
        """베이스 Qwen 모델로 답변 생성 (파인튜닝 없이 베이스 모델만 사용)"""
...
def main():
    """메인 실행 함수 - 베이스 모델 버전 (대회 형식)"""
    
    # 베이스 Qwen RAG 추론기 초기화
    print("베이스 Qwen 모델 로드 중...")
    qwen_rag = QwenBaseRAGInference()
    
    # 테스트 데이터 로드 (predict.py와 동일)
    test_path = "/content/BTM_25Q3_GEN/data/korean_language_rag_V1.0_test.json"
    ...
    output_path = "predictions-base.json"
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    
    print(f"✅ predictions-base.json 생성 완료! (RAG Base 모델)")
